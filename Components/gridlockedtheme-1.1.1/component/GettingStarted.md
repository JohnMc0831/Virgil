...
```csharp
GridlockedTheme.Apply (UIProgressView.Appearance);
```
...
the following example, we apply `GridlockedTheme` to `UIProgressView` only
when `UIProgressView` is a child of `UINavigationBar`:

```csharp
GridlockedTheme.Apply (UIProgressView.AppearanceWhenContainedIn (typeof (UINavigationBar)));
```
...
GridlockedTheme.Apply (tableView);
```
